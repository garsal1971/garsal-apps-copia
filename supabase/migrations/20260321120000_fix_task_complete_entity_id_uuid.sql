-- ============================================================
-- Migration: fix entity_id uuid = text in task_complete
-- Data: 2026-03-21
--
-- Causa radice dell'errore "operator does not exist: uuid = text":
--   La colonna cm_notification_rules.entity_id è di tipo UUID nel DB
--   (non TEXT come indicato nelle migration files originali).
--   La funzione task_complete usava entity_id = p_task_id::text
--   ovvero UUID = TEXT → errore PostgreSQL 42883.
--
-- Fix: sostituire entity_id = p_task_id::text con entity_id = p_task_id
--      (UUID = UUID) in tutte le clausole WHERE della funzione.
-- ============================================================
...
BEGIN
  ...
  IF v_task.type = 'single' THEN
    ...
    DELETE FROM cm_notification_rules
     WHERE entity_id = p_task_id      -- UUID = UUID (fix: rimosso ::text)
       AND app = 'tasks';

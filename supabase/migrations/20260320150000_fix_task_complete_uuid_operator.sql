-- ============================================================
-- Migration: fix "operator does not exist: uuid = text"
-- Data: 2026-03-20
--
-- Problema: se in precedenza era stata creata una funzione
--   task_complete(text, date) o task_complete(text, text),
--   PostgreSQL la preferisce a task_complete(uuid, date) quando
--   il client passa p_task_id come stringa JSON (text).
--   Il corpo di quella vecchia funzione fa WHERE id = p_task_id
--   dove id è uuid e p_task_id è text → errore 42883
--   "operator does not exist: uuid = text".
--
-- Fix:
--   1. Drop di eventuali overload text della funzione
--   2. Ricrea task_complete(uuid, date) per garantire la versione corretta
-- ============================================================

-- 1. Rimuovi eventuali overload con parametri text
DROP FUNCTION IF EXISTS task_complete(text, date);
DROP FUNCTION IF EXISTS task_complete(text, text);

-- 2. Ricrea la funzione con i tipi corretti (idempotente)
...
BEGIN
  ...
  IF v_task.type = 'single' THEN
    ...
    -- Record history: terminazione automatica (allineato a completeTask() JS)
    INSERT INTO ts_history (task_id, from_status, to_status, action, points, timestamp)

-- ============================================================
-- Migration: task_complete — funzione unificata per web e Telegram
-- Data: 2026-03-13
--
-- Consolida la logica di completamento task in un'unica RPC
-- chiamata sia da completeTask() in tasks.html (web) che dal
-- telegram-webhook (bottone "✅ Fatto").
--
-- Rispetto a task_complete_via_telegram aggiunge:
--   • record history 'terminated' per task single (allineato al JS)
--
-- La vecchia funzione task_complete_via_telegram viene rimossa.
-- ============================================================
...
-- Rimuovi la vecchia funzione (sostituita da task_complete)
DROP FUNCTION IF EXISTS task_complete_via_telegram(uuid, date);
